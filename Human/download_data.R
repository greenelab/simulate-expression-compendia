...
# TCGA, SRA and GTEx
  library('recount')
  source("~/Documents/Repos/simulate-expression-compendia/functions/calculations.R")

## Metadata

# Download the metadata for all samples in recount, and write it to a file
metadata <- all_metadata()
write.table(metadata, '~/Documents/Repos/simulate-expression-compendia/Human_test/data/metadata/recount2_metadata.tsv', sep='\t', row.names=FALSE)
...
## Save counts matrix to file
  write.table(data_counts_all_rpkm,
            '~/Documents/Repos/simulate-expression-compendia/Human_test/data/input/recount2_gene_RPKM_data_test.tsv',
            sep='\t',
            row.names=TRUE,
            col.names=NA)
