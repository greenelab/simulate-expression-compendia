...
g_correct = ggplot(all_data_df, aes(x='PC1', y='PC2')) + geom_point(aes(color='correction'), alpha=0.2) + geom_point(df_layer_1, aes(color=['before']), alpha=0.2) + geom_point(df_layer_2, aes(color=['after']), alpha=0.2) + facet_wrap('~num_experiments_cat') + labs(x = "PC 1", y = "PC 2", title = "PCA after correcting for experiment variation") + theme_bw() + theme(
    legend_title_align = "center",
    plot_background=element_rect(fill='white'),
    legend_key=element_rect(fill='white', colour='white'), 
    plot_title=element_text(weight='bold')) \
...
g_correct = ggplot(all_data_df, aes(x='PC1', y='PC2')) + geom_point(aes(color='correction'), alpha=0.2) + geom_point(df_layer_1, aes(color=['before']), alpha=0.2) + geom_point(df_layer_2, aes(color=['after']), alpha=0.2) + facet_wrap('~num_experiments_cat') + labs(x = "PC 1", y = "PC 2", title = "PCA after correcting for experiment variation") + guides(colour=guide_legend(override_aes={'alpha': 1})) + scale_colour_manual(["grey", '#87CEFA']) + theme(
        plot_background=element_rect(fill='black'),
        legend_title_align = "center",
        legend_background=element_rect(fill='black', colour='black'),
        legend_key=element_rect(fill='black', colour='black'), 
