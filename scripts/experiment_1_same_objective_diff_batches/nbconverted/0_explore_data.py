        ...
        treatment=(
            list( 
                map(
                    lambda x:
                    'NA' if type(x) == float
                    else values[0] if values[0].lower() in x.lower().strip()
                    else values[1] if values[1].lower() in x.lower().strip()
                    else values[2] if values[2].lower() in x.lower().strip()
                    else values[3] if values[3].lower() in x.lower().strip()
                    else values[4] if values[4].lower() in x.lower().strip()
                    else values[5] if values[5].lower() in x.lower().strip()
                    else values[6] if values[6].lower() in x.lower().strip()
                    else values[7] if values[7].lower() in x.lower().strip()
                    else values[8] if values[8].lower() in x.lower().strip()
                    else values[9] if values[9].lower() in x.lower().strip()
                    else values[10] if values[10].lower() in x.lower().strip()
                    else values[11] if values[11].lower() in x.lower().strip()
                    else 'NA',
                    metadata_vector
                )      
            )
...
# Save mapping
metadata_selected_labeled.to_csv(mapping_file, sep="\t")
...
# Get counts
metadata_selected_labeled[metadata_selected_labeled[metadata_field] == 'tobramycin'].shape
...
ggplot(aes(x='1',y='2', color='metadata'), data=embedding_df) +         geom_point(alpha=0.5) +         scale_color_brewer(type='qual', palette=color_theme)
...
# Plot
ggplot(aes(x='1',y='2', color='metadata'), data=embedding_normalized_df) +     geom_point(alpha=0.5) +     scale_color_brewer(type='qual', palette=color_theme)
